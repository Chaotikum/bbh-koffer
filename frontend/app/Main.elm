...
update msg model =
  case (log "msg" msg) of
    ...
    Start ->
      ( { model |
        page = if List.isEmpty model.things then "start" else "suitcase"
      }, Cmd.none )
    ...
    Submit ->
      if List.length model.things > 5 then
        ...
        let stats = nextStats model.stats <| List.map (\t -> t.uuid) model.things
        in (
          { model |
            page = "end",
            stats = stats
        ...
        case getThing uuid of
          Just t ->
            ( { model |
              ...
              page = if model.page /= "end" then "suitcase" else model.page
            }, Cmd.none )
          Nothing -> ( model, Cmd.none )

    Remove uuid ->
      let
          things = List.filter (\t -> t.uuid /= uuid) model.things
          page = if List.length things == 0 then "start" else model.page
      in
          ( { model |
            page = page,
            things = things,
...
view model =
  div [] [
    div [class "icons"] [],
    case model.page of
      "start" -> pageStart model
      "suitcase" -> pageSuitcase model
      "end" -> pageEnd model
      _ -> span [] [text ("unknown page: " ++ model.page)]
  ]
...
pageSuitcase model =
  hero [] [
    header [
      span [
        class <| "counter" ++ if List.length model.things > 5 then " is-danger" else ""
        ...
        span [class "subtitle"] [text "Gegenständen im Koffer:"]
          ...
          [
            a [
              onClick Submit,
              class <| "button is-medium arrow-after arrow-small" ++ if List.length model.things > 5 then " is-disabled" else ""
              ] [text "zur Auswertung"]
    ],
    selectedThingBox
  ...
  [
    ...
    div [class "thing-select"] <|
      ...
      (
        \t -> a [
          onClick <| Show t.uuid,
          class <| "button is-medium" ++ if t.uuid == model.selected then " is-active" else ""
        ] [text t.name]
      )
      model.things
  ]
...
statRow stats thing =
  let
    count = List.filter (\entry -> thing.uuid == (fst entry)) stats.things
      |> List.head
      |> Maybe.withDefault ("", 1)
      |> snd
    percent = round <| (count * 100) / (toFloat stats.total)
    percentStr = (toString percent) ++ "%"
    labelStr = (padInt count) ++ " Mal = " ++ (toString percent) ++ "%"
    barContent =
      if percent > 75 then
        [
...
hero h c f =
  let
      header = if List.isEmpty h then [] else [div [class "hero-header"] h]
      content = if List.isEmpty c then [] else [div [class "hero-content"] [
        div [class "container"] c
      ]]
      footer = if List.isEmpty f then [] else [div [class "hero-footer"] [
        div [class "container"] f
      ]]
      children = List.foldr (\el ch -> if List.isEmpty el then ch else el ++ ch) [] [header, content, footer]
  in
...
nextStats stats uuids =
  { stats |
    ...
    things = List.foldr
      (\uuid things ->
        let
          existing = List.head <| List.filter (\t -> (fst t) == uuid) things
